...
# Build the game.
odin build src -define:GameProfile=true -define:DEV_MODE=true -use-separate-modules -extra-linker-flags:"$EXTRA_LINKER_FLAGS" -show-timings -define:RAYLIB_SHARED=true -build-mode:dll -out:game_tmp$DLL_EXT -debug $VET
...
if ! pgrep game.bin > /dev/null; then
    odin build main_hot_reload -define:GameProfile=true -define:DEV_MODE=true -use-separate-modules -out:game.bin $VET -debug
fi
